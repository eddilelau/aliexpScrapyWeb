        ...
        if not os.path.exists(file_path):
            ...
            if response.status_code == 200:
                with open(file_path, 'wb') as f:
                    ...
                    f.close()
                    print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()),'下载图片{}'.format(file_path))

    except RequestException:
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()),'请求图片错误', url)

def insertToCompetingProductDailySales(productInfo):
    competingProductDailySales.objects.bulk_create(productInfo)
...
def updateToCompetingProductDailySalesforFiveDays():
    ...
    past1_data={productInfo['productId']:productInfo['totalSales'] for productInfo in competingProductDailySales.objects.filter(date=past1_date).values('productId','totalSales')}
    past2_data={productInfo['productId']:productInfo['totalSales'] for productInfo in competingProductDailySales.objects.filter(date=past2_date).values('productId','totalSales')}
        ...
        if infringeProductNum:
            ...
            if infringeProductData.updateDate == datetime.date.today() - datetime.timedelta(days=1):
                if infringeProductData.confirm_times >= 2:
                    ...
                    competingProductInfo.objects.filter(productId=product_id).delete()
                else:
                    infringeProductinfo.objects.filter(productId=product_id).update(
                        totalSales=productData.totalSales,
                        totalEvaluation=productData.totalEvaluation,
...
def main(productIdlist):
    ...
    while productIdlist:
        ...
        sessionNum+=1
    insertToCompetingProductDailySales(productInfo)
    updateToCompetingProductDailySalesforFiveDays()
