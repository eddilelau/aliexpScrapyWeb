...
def send_mail(file_name,dateToday):
    ...
    try:
        ...
        message['From'] = Header("Python_8号店铺评价_"+dateToday, 'utf-8')
        message['To'] = Header("工作邮箱", 'utf-8')
        subject = 'Python_8号店铺评价_'+dateToday
        message['Subject'] = Header(subject, 'utf-8')
        message.attach( MIMEText('Python_8号店铺评价_'+dateToday, 'plain', 'utf-8'))


        # 构造附件1，传送当前目录下的 test.txt 文件
        att1 = MIMEText(open('/root/venv/ali_feebackForShoop_Data/Num8/' + file_name, 'rb').read(), 'base64', 'utf-8')
...
def getFeedback(i,file_name):
    ...
    with open('/root/venv/ali_feebackForShoop_Data/Num8/'+file_name,'a+',encoding='utf-8') as f:
        for item in items:
