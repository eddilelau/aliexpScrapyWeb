...
def send_mail(file_name,dateToday):
    ...
    try:
        ...
        message['From'] = Header("Python_7号店铺评价_"+dateToday, 'utf-8')
        message['To'] = Header("工作邮箱", 'utf-8')
        subject = 'Python_7号店铺评价_'+dateToday
        message['Subject'] = Header(subject, 'utf-8')
        message.attach( MIMEText('Python_7号店铺评价_'+dateToday, 'plain', 'utf-8'))


        # 构造附件1，传送当前目录下的 test.txt 文件
        att1 = MIMEText(open('/root/venv/ali_feebackForShoop_Data/Num7/' + file_name, 'rb').read(), 'base64', 'utf-8')
...
def getFeedback(i,file_name):
    ...
    with open('/root/venv/ali_feebackForShoop_Data/Num7/'+file_name,'a+',encoding='utf-8') as f:
        for item in items:
...
def main():
    ...
    file_name = "Num7_ShoopFeedback"+dateToday+".txt"  #输出爬虫数据
    for i in range(30):
